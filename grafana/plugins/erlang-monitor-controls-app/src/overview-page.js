...
import { prometheusSamples, safeMarkdownText } from './holmes-logic.js';
import {
...
function AlertList({ alerts, dashboardUID, server }) {
  ...
  const holmesBase = new URLSearchParams({ dashboard_uid: dashboardUID, server, from: source.get('from') || 'now-6h', to: source.get('to') || 'now' });
  return h('section', { className: 'eo-panel' },
    ...
    alerts.length ? alerts.map((alert, index) => {
      ...
      const params = new URLSearchParams(holmesBase);
      ...
      return h('details', { className: 'eo-alert', key: alert.fingerprint, open: index === 0 },
        ...
        h('div', { className: 'eo-alert-body' },
          ...
          h('div', { className: 'eo-alert-line' }, h('div', { className: 'eo-alert-label' }, '辅助分析'), h('a', { className: 'eo-holmes', href: `/a/erlang-monitor-controls-app/ops-agent?${params.toString()}` }, '打开运维 Agent'))));
